...
BEGIN
	...
	-- Single Source?
	ELSE IF @fact_priority_list_source_count_02 = 1 AND (@dim_product_clk_mtl + @dim_product_dtp_mtl + @dim_product_sjd_mtl) <= 1 AND @dim_factory_name_priority_list_primary_02 = 'ICC'
	BEGIN
		SET @dim_factory_id_original_unconstrained_02 = (SELECT [id] FROM [dbo].[dim_factory] WHERE [short_name] = @dim_factory_name_priority_list_primary_02)
		SET @allocation_logic = @allocation_logic +' => ' + 'Single Source'
	END

	ELSE IF @fact_priority_list_source_count_02 = 1 AND (@dim_product_clk_mtl + @dim_product_dtp_mtl + @dim_product_sjd_mtl) <= 1 AND @dim_factory_name_priority_list_primary_02 = 'DTC'
	BEGIN
		SET @dim_factory_id_original_unconstrained_02 = (SELECT [id] FROM [dbo].[dim_factory] WHERE [short_name] = @dim_factory_name_priority_list_primary_02)
		SET @allocation_logic = @allocation_logic +' => ' + 'Single Source'
	END

	ELSE IF @fact_priority_list_source_count_02 = 1 AND (@dim_product_clk_mtl + @dim_product_dtp_mtl + @dim_product_sjd_mtl) <= 1 AND @dim_factory_name_priority_list_primary_02 = 'SJV'
	BEGIN
		SET @dim_factory_id_original_unconstrained_02 = (SELECT [id] FROM [dbo].[dim_factory] WHERE [short_name] = 'HSC')
		SET @allocation_logic = @allocation_logic +' => ' + 'Single Source'
	END
	...
	-- 1st priority = SJV?
	ELSE IF @dim_factory_name_priority_list_primary_02 = 'SJV'
	BEGIN
		SET @dim_factory_id_original_unconstrained_02 = (SELECT [id] FROM [dbo].[dim_factory] WHERE [short_name] = 'HSC')
		SET @allocation_logic = @allocation_logic +' => ' + '1st priority = SJV' +' => ' +'1st priority'
		IF @dim_factory_name_priority_list_secondary_02 IS NOT NULL
		BEGIN
